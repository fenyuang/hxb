      ...
      <table cellspacing="0" cellpadding="2" border="1" >
         <tr v-for="list in ClueMan">
           ...
           <td class="Clue_hierarchy_num">
             <span class="clue">线索</span>
             <span class="num">{{list.clue_count}}</span>
           </td>
           <td class="Clue_hierarchy_btn">
             <input type="button"  value="移除" @tap="remove(list.id)"/>
             <input type="button" class="dele" value="删除" @tap="dele(list.id)"/>
           </td>
         </tr>
      </table>

    <!--添加按钮-->
    <div class="add_btn" @tap="AddClueGra">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      remove (Clueid) {
        this.$router.push({path: '/RemoveClue', query: {id: Clueid}})
      },
      dele (Clueid) {
        axios.post('http://hxb.scpoo.com/hxb/index.php/index/clue/clue_grade_delete', {id: Clueid}).then((res) => {
          if (res.data.ret === 100) {
            alert(res.data.msg)
